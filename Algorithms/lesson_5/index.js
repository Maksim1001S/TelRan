const main = () => {
    ...
    const A = Array.from({ length: count + 1 }, () => Array(maxWeights + 1).fill(0));

    for (let k = 1; k <= count; k++) {
        for (let s = 1; s <= maxWeights; s++) {
            if (weights[k - 1] <= s) {
    ...
    const result = [];
    trace(A, weights, count, maxWeights, result)
...
const trace = (A, weights, k, s, result) => {
    if (k === 0 || s === 0) {
        return;
    }
